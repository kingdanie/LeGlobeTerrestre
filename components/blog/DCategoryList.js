...
import DCategoryItem from "./DCategoryItem";
...
export default function DCategoryList({ categoryName, name={name}, data, isAll }) {
  ...
  return (
    <>
      <div className="my-8">
        <LeftAccentedH2Text text={removeDashFromTitles(categoryName) || ""} isCapitalize />
      </div>
        ...
        {postsData ? (
          <>
            {postsData.map((post, i) => {
              return (
                <>
                  <DCategoryItem name={name} key={i} post={post} />
                  {(i + 1) % 5 == 0 && <CategoryHorizontalAdSpot key={i+20} />}{" "}
                </>
              );
            })}
