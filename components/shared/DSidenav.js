...
export default function DSidenav({ menuData }) {
  const { isNavOpen, setNavOpenState } = useSideMenu();

  const router = useRouter();

  const { headerMenus, name } = menuData || {};
  ...
  const prefixPath = (path) => {
    if (pathname.includes(`${name}/category`)) {
      return `${path}-${name}`;
    }
