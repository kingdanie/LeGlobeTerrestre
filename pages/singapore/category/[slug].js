...
import {
  GET_ALL_SINGAPORE_CATEGORY_PAGE,
  SINGAPORE_CATEGORY_SLUGS,
...
export default function SingaporeCategoryPage({ data = {}, footerMenus, categoryName }) {
  return (
...
export async function getStaticProps({ params }) {
  ...
  const { data, error } = await  client.query({
    query: slug == "all" ? GET_ALL_SINGAPORE_CATEGORY_PAGE : GET_CATEGORY_PAGE,
    variables: {
      initialCount: 10,
      categorySlug: slug,
    },
