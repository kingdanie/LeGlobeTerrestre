...
import {
  GET_ALL_DUBAI_CATEGORY_PAGE,
  DUBAI_CATEGORY_SLUGS,
...
export default function DubaiCategoryPage({ data = {}, footerMenus, categoryName }) {
  return (
...
export async function getStaticProps({ params }) {
  ...
  const { data, error } = await  client.query({
    query: slug == "all" ? GET_ALL_DUBAI_CATEGORY_PAGE : GET_CATEGORY_PAGE,
    variables: {
      initialCount: 10,
      categorySlug: slug,
    },
