...
export default function BaliCategoryPage({ data = {}, footerMenus, categoryName }) {
  return (
...
export async function getStaticProps({ params }) {
  ...
  const { data, error } = await  client.query({
    query: slug == "all" ? GET_ALL_BALI_CATEGORY_PAGE : GET_CATEGORY_PAGE,
    variables: {
      initialCount: 10,
      categorySlug: slug,
    },
