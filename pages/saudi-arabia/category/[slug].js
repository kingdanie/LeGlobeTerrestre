...
import {
  GET_ALL_SAUDI_ARABIA_CATEGORY_PAGE,
  SAUDI_ARABIA_CATEGORY_SLUGS,
...
export default function SaudiCategoryPage({ data = {}, footerMenus, categoryName }) {
  return (
...
export async function getStaticProps({ params }) {
  ...
  const { data, error } = await  client.query({
    query: slug == "all" ? GET_ALL_SAUDI_ARABIA_CATEGORY_PAGE : GET_CATEGORY_PAGE,
    variables: {
      initialCount: 10,
      categorySlug: slug,
    },
...
export async function getStaticPaths() {
  const { data, error } = await client.query({
    query: SAUDI_ARABIA_CATEGORY_SLUGS,
  });
