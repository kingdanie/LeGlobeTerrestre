...
import {
  GET_ALL_JAPAN_CATEGORY_PAGE,
  JAPAN_CATEGORY_SLUGS,
...
export default function JapanCategoryPage({ data = {}, footerMenus, categoryName }) {
  return (
...
export async function getStaticProps({ params }) {
  ...
  const { data, error } = await  client.query({
    query: slug == "all" ? GET_ALL_JAPAN_CATEGORY_PAGE : GET_CATEGORY_PAGE,
    variables: {
      initialCount: 10,
      categorySlug: slug,
    },
